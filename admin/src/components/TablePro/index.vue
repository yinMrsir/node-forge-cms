  ...
  <div>
    ...
    >
      <template v-for="item in filterOptions">
        <el-form-item :label="item.title" :prop="item.field" v-bind="item.searchProps">
          <template v-if="item.formSlot">
            <slot :name="item.formSlot" v-bind="{ queryParams }"></slot>
          </template>
          <template v-else-if="item.type === 'select' || item.type === 'radio'">
            <el-select
              v-model="queryParams[item.searchField || item.field]"
              :placeholder="`请选择${item.title}`"
              v-bind="item.searchFormProps"
              clearable
            >
          ...
          <template v-else>
            <el-input v-model="queryParams[item.searchField || item.field]" :placeholder="`请输入${item.title}`" />
          </template>
        </el-form-item>
      </template>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5" v-if="isShowButtonList">
        <el-button v-if="formOptions.length > 0" type="primary" plain icon="Plus" @click="handleAdd">新增</el-button>
      </el-col>
      <el-col :span="1.5" v-if="isShowButtonList">
      ...
      <el-col :span="1.5" v-for="(item, index) in buttons">
        <el-button :key="index" :type="item.type" :plain="item.plain" :icon="item.icon" @click="item.click">
          {{ item.text }}
        </el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleSelectionChange">
      ...
      <el-table-column v-if="isShowIdColumn" :prop="isShowIdColumn" width="60" label="ID" align="center" />
      <template v-for="item in TableOptions">
          ...
          <el-table-column label="操作" align="center" v-bind="item.props">
            <template #default="scope">
              <el-button
                link
                type="primary"
                v-show="typeof act?.hidden === 'function' ? !act.hidden(scope.row) : true"
          ...
          <el-table-column :prop="item.field" :label="item.title" v-bind="item.props">
            <template #default="scope">
              <el-image
                ...
                "
                :style="item.style || { width: '40px', maxHeight: '100px' }"
                fit="contain"
              />
            </template>
          </el-table-column>
        ...
        <template v-else-if="item.type === 'radio' || item.type === 'select'">
          <el-table-column :prop="item.field" :label="item.title" v-bind="item.props">
            <template #default="scope">
              <dict-tag :options="item.options" :value="getValueByPath(scope.row, item.field)" />
            </template>
          </el-table-column>
        </template>
        <template v-else-if="item.type === 'date' || item.type === 'dateTime'">
          <el-table-column :prop="item.field" :label="item.title" v-bind="item.props">
            <template #default="scope">
              <span>
                {{ parseTime(scope.row[item.field], item.type === 'date' ? '{y}-{m}-{d}' : item.dateFormat) }}
              </span>
            </template>
          </el-table-column>
        </template>
    ...
    <el-dialog :title="title" v-model="open" :width="formDialogWidth" append-to-body :close-on-click-modal="false">
      <el-form ref="formRef" :model="form" label-width="120px" :rules="rules">
        <el-form-item :label="`${item.title}：`" :prop="item.field" v-for="item in formOptions">
          <template v-if="item.formSlot">
            <slot :name="item.formSlot" v-bind="form"></slot>
          </template>
          <template v-else-if="item.type === 'date' || item.type === 'dateTime'">
            <el-date-picker
          ...
          <template v-else-if="item.type === 'radio'">
            <el-radio-group v-model="form[item.field]" v-bind="item.formProps" @change="item.change">
              <el-radio :label="radio.value" v-for="(radio, index) in item.options" :key="index">
                {{ radio.label }}
              </el-radio>
            </el-radio-group>
          </template>
          ...
          <template v-else-if="item.type === 'cascader'">
            <el-cascader :options="item.options || []" v-model="form[item.field]" v-bind="item.formProps" />
          </template>
          <template v-else-if="item.type === 'img'">
            <image-upload type="flag" :limit="1" v-model:modelValue="form[item.field]" />
          </template>
          ...
          <template v-else-if="item.type === 'richText'">
            <rich-text-editor v-model="form[item.field]"></rich-text-editor>
          </template>
          <template v-else>
...
<script setup>
  ...
  function getValueByPath(obj, path) {
    ...
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i];
      // 如果当前对象存在且具有该属性，则继续访问下一级属性
      if (current && typeof current === 'object' && key in current) {
        current = current[key];
      } else {
        // 如果对象不存在该属性，则返回 undefined
        return undefined;
      }
    }
